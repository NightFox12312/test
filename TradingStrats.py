...
def fibMACD(Trade_Direction, Close, Open, High, Low, MACD_signal, MACD, EMA200, current_index, stop_loss_val, take_profit_val, highest_swing, lowest_swing):
    period = 500  ##Record peaks and troughs in last period timesteps
...
def calculate_elliott_wave_label(Close):
    labels = []
    increasing_segment = False
    start_index = 0
    for i in range(1, len(Close)):
        if Close[i] > Close[i-1] and not increasing_segment:
            increasing_segment = True
            start_index = i-1
        elif Close[i] < Close[i-1] and increasing_segment:
            increasing_segment = False
            labels[start_index:i] = ["upward"] * (i - start_index)
            labels[i-1] = "downward"
    labels[start_index:] = ["upward"] * (len(Close) - start_index)
    return labels




def calculate_fibonacci_levels(Close, highest_swing, lowest_swing, ratios):
    levels = []
    max_level = Close[highest_swing]
    min_level = Close[lowest_swing]
    for ratio in ratios:
...
def breakout(Trade_Direction, CurrentPos, clos_pos, high, low, Close, VolumeStream, max_Close, min_Close, max_Vol, current_index, highest_swing, lowest_swing, elliott_wave_label_upward, elliott_wave_label_downward,fastd, fastk, RSI, MACD, macdsignal, Open):
    ratios = [0.236, 0.382, 0.5, 0.618, 0.786]
    ratios1 = [0, 0.236, 0.382, 0.5, 0.618, 0.786, 1]

    fibonacci_levels = calculate_fibonacci_levels(Close, highest_swing, lowest_swing, ratios)
    fibonacci_levels1 = calculate_fibonacci_levels(Close, highest_swing, lowest_swing, ratios1)
    
    percentage = 0.00005  # Phần trăm chấp nhận được
    
    minimum_difference = float('inf')  # Initialize with a large value
    closest_level = None

    for level in fibonacci_levels:
        difference = abs(Close[current_index] - level)
        if difference < minimum_difference:
            minimum_difference = difference
            closest_level = level

    acceptable_range = percentage * closest_level

    if minimum_difference < acceptable_range:
        if ((fastd[current_index] < 20 and fastk[current_index] < 20 and RSI[current_index] > 50 and MACD[current_index] > macdsignal[current_index] and MACD[current_index - 1] < macdsignal[
            current_index - 1]) or
            (fastd[current_index - 1] < 20 and fastk[current_index - 1] < 20 and RSI[current_index] > 50 and MACD[current_index] > macdsignal[current_index] and MACD[current_index - 2] < macdsignal[
                current_index - 2] and fastd[current_index] < 80 and fastk[current_index] < 80) or
    ...
    return Trade_Direction, fibonacci_levels1



# def breakout(Trade_Direction,CurrentPos, clos_pos, high, low, Close, VolumeStream, max_Close, min_Close, max_Vol, current_index, highest_swing, lowest_swing):
#     window_size = 2584
#     if current_index >= window_size:
#         mean_Close = sum(Close[current_index-window_size:current_index]) / window_size
#     else:
#         mean_Close = sum(Close[:current_index]) / current_index if current_index > 0 else Close[current_index]

#     # Determine the current market trend based on the slope of the 50-day moving average
#     trend = 1 if Close[current_index] > mean_Close else -1

#     ratios = [0.236, 0.382, 0.5, 0.618, 0.786]
#     # ratios = [0.382, 0.5, 0.618]
#     levels = calculate_fibonacci_levels(Close, highest_swing, lowest_swing, ratios)
#     if Close[current_index] in levels:
#         if Close[current_index] < Close[current_index - 1] and Close[current_index] < min_Close.iloc[current_index-1] and VolumeStream[current_index] > max_Vol.iloc[current_index-1] and trend == -1:
#             Trade_Direction = 1  # Open a long position
#         elif Close[current_index] > Close[current_index - 1] and Close[current_index] > max_Close.iloc[current_index-1] and VolumeStream[current_index] > max_Vol.iloc[current_index-1] and trend == 1:
#             Trade_Direction = 0  # Open a short position

#     return Trade_Direction


# def breakout(Trade_Direction,CurrentPos, clos_pos, high, low, Close, VolumeStream, max_Close, min_Close, max_Vol, current_index):
#     clos_pos = 1
#     if CurrentPos == -99:
#         Trade_Direction = -99
#         # Calculate the 50-day moving average of Close
#         window_size = 2584
#         if current_index >= window_size:
#             mean_Close = sum(Close[current_index-window_size:current_index]) / window_size
#         else:
...
#         # Determine the trading direction based on breakout conditions and trend confirmation
#         if trend == 1:
#             # Look for buying breakout opportunities
#             if Close[current_index] > max_Close.iloc[current_index-1] and VolumeStream[current_index] > max_Vol.iloc[current_index-1]:
#                 # Check if there is no existing long position
#                 if Trade_Direction != 0:
#                     Trade_Direction = 0  # Enter a long position
#                 else:
#                     Trade_Direction = Trade_Direction  # Maintain the current long position
#             else:
#                 # Check if there is an existing long position
#                 if Trade_Direction == 0:
#                     Trade_Direction = 1  # Exit the long position
#                 # else:
...
#         elif trend == -1:
#             # Look for selling breakout opportunities
#             if Close[current_index] < min_Close.iloc[current_index-1] and VolumeStream[current_index] > max_Vol.iloc[current_index-1]:
#                 # Check if there is no existing short position
#                 if Trade_Direction != 1:
#                     Trade_Direction = 1  # Enter a short position
#                 else:
#                     Trade_Direction = Trade_Direction  # Maintain the current short position
#             else:
#                 # Check if there is an existing short position
#                 if Trade_Direction == 1:
#                     Trade_Direction = 0  # Exit the short position
#                 # else:
#                 #     if Close[current_index] >= max_Close.iloc[current_index] and VolumeStream[current_index] >= max_Vol.iloc[current_index]:
#                 #         Trade_Direction = 0
#                 #     elif Close[current_index] <= min_Close.iloc[current_index] and VolumeStream[current_index] >= max_Vol.iloc[current_index]:
...
def calculate_take_profit_threshold(volume):
    std_volume = np.std(volume)  # Calculate the standard deviation of volume
    take_profit_multiplier = 2  # Example: Multiplier of 2 times the standard deviation
    take_profit_threshold = take_profit_multiplier * std_volume
    return take_profit_threshold

def calculate_stop_loss_threshold(volume):
    std_volume = np.std(volume)  # Calculate the standard deviation of volume
    stop_loss_multiplier = 1.5  # Example: Multiplier of 1.5 times the standard deviation
    stop_loss_threshold = stop_loss_multiplier * std_volume
    return stop_loss_threshold


# def breakout(Trade_Direction, high, low, Close, VolumeStream, max_Close, min_Close, max_Vol, current_index):
#  # Calculate the 50-day moving average of Close
#     window_size = 2584
#     if current_index >= window_size:
#         mean_Close = sum(Close[current_index-window_size:current_index]) / window_size
...
def TripleScreenTradingSystem(trade_direction, MACD, RSI, sma_indicator, ema_indicator, bollinger_hband, bollinger_lband, force_index, macdsignal, current_index):
    # # First Screen: MACD
    # macdsignal_window = 20  # Number of previous macdsignal values to consider
    # macdsignal_mean = np.mean(macdsignal[current_index - macdsignal_window + 1: current_index + 1])  # Mean of previous macdsignal values
    # macdsignal_std = np.std(macdsignal[current_index - macdsignal_window + 1: current_index + 1])  # Standard deviation of previous macdsignal values
    # macd_threshold = macdsignal_mean + 2 * macdsignal_std  # Adjusted MACD threshold based on the mean and standard deviation

    # if MACD[current_index] > macd_threshold:
    #     trade_direction = 1  # Set trade direction to 1 (long)
    # elif MACD[current_index] < -macd_threshold:
    #     trade_direction = 0  # Set trade direction to 0 (short)
        # First Screen: MACD
    if MACD[current_index] > 0.2:  # Adjusted MACD threshold
        trade_direction = 1  # Set trade direction to 1 (long)
    elif MACD[current_index] < -0.2:  # Adjusted MACD threshold
        trade_direction = 0  # Set trade direction to 0 (short)


    # Second Screen: RSI with dynamic thresholds
    rsi_upper_threshold = 100 - ((100 - RSI[current_index-1]) * 0.1)  # Dynamic upper threshold based on previous RSI
    rsi_lower_threshold = (RSI[current_index-1] * 0.1)  # Dynamic lower threshold based on previous RSI

    if trade_direction == 1 and RSI[current_index] > rsi_upper_threshold:
        trade_direction = 0  # Set trade direction to 0 (short)
    elif trade_direction == 0 and RSI[current_index] < rsi_lower_threshold:
        trade_direction = 1  # Set trade direction to 1 (long)

    # Third Screen: Moving Averages, Bollinger Bands, and Force Index
    sma_ema_threshold = (sma_indicator[current_index] - ema_indicator[current_index]) * 0.5  # Adjusted threshold
    bollinger_width_threshold = (bollinger_hband[current_index] - bollinger_lband[current_index]) * 0.1  # Adjusted threshold
    force_index_threshold = force_index[current_index-1] * 0.1  # Adjusted threshold

    if trade_direction == 1:
        if sma_indicator[current_index] < ema_indicator[current_index] - sma_ema_threshold or sma_indicator[current_index] > bollinger_hband[current_index] + bollinger_width_threshold or force_index[current_index] < -force_index_threshold:
            trade_direction = 0  # Set trade direction to 0 (short)
    elif trade_direction == 0:
        if sma_indicator[current_index] > ema_indicator[current_index] + sma_ema_threshold or sma_indicator[current_index] < bollinger_lband[current_index] - bollinger_width_threshold or force_index[current_index] > force_index_threshold:
            trade_direction = 1  # Set trade direction to 1 (long)

    return trade_direction  # Return the trade direction (1 for long, 0 for short, or None)

def volume_profile_order_flow_strategy(trade_direction, MACD, RSI, stochrsi_k, stochrsi_d, macdsignal, current_index):
    # Apply trading rules based on the indicators and volume profile/order flow
    if MACD[current_index] > macdsignal[current_index] and RSI[current_index] > 70 and stochrsi_k[current_index] > stochrsi_d[current_index]:
        # Enter a long position
        trade_direction = 1
    elif MACD[current_index] < macdsignal[current_index] and RSI[current_index] < 30 and stochrsi_k[current_index] < stochrsi_d[current_index]:
        # Enter a short position
        trade_direction = 0

    return trade_direction

def EnhancedTripleScreenTradingSystem(trade_direction, high, low, Close, RSI, stochrsi_k, stochrsi_d, sma_indicator, ema_indicator, bollinger_hband, bollinger_lband, force_index, mfi, current_index):
    # Calculate consecutive occurrences of high or low prices
    consecutive_high_counts = 0
    consecutive_low_counts = 0

    for i in range(current_index, 0, -1):
        if high[i] == high[i-1]:
            consecutive_high_counts += 1
        else:
            break
    ...
    # Determine resistance and support levels if consecutive counts are greater than 3
    if consecutive_high_counts > 21:
        resistance_level = high[current_index - consecutive_high_counts]

    if consecutive_low_counts > 21:
        support_level = low[current_index - consecutive_low_counts]

    # Determine the trade direction based on the current price level
    if high[current_index] == resistance_level:
        trade_direction = 1  # Short position
    elif low[current_index] == support_level:
        trade_direction = 0  # Long position
...
# Hàm tìm điểm swing cao nhất
def find_highest_swing(candles, peaks):
    highest_swing = max(candles)
    for peak in peaks:
        if peak > highest_swing and peak != 0:
            highest_swing = peak
    return highest_swing

# Hàm tìm điểm swing thấp nhất
def find_lowest_swing(candles, troughs):
    lowest_swing = min(candles)
    for trough in troughs:
        if trough < lowest_swing and trough != 0:
            lowest_swing = trough
    return lowest_swing

def support(ATR,stop_loss_val_arr, take_profit_val_arr,stop_loss_val_arr1, take_profit_val_arr1, peaks, troughs, Close, High, Low, Trade_Direction, SL, TP, TP_SL_choice, current_index, resistance_level, support_level):
...
def calculate_average_true_range(Close, High, Low):
    true_ranges = []
    for i in range(1, len(Close)):
        high = High[i]
        low = Low[i]
...
def SetSLTP(ATR,stop_loss_val_arr, take_profit_val_arr,stop_loss_val_arr1, take_profit_val_arr1, peaks, troughs, Close, High, Low, Trade_Direction, SL, TP, TP_SL_choice, current_index, resistance_level, support_level, highest_swing, lowest_swing, Open, fibonacci_levels1):
    ...
    if TP_SL_choice == 'en':
        ...
        for level in fibonacci_levels1:
            difference = abs(Close[current_index] - level)
            if difference < minimum_difference:
                minimum_difference = difference
                closest_level = level
        ...
        if minimum_difference < acceptable_range:
            if Close[current_index] >= closest_level:
            
                fib_index = levels.index(Close[current_index] - abs(Close[current_index] - closest_level))
            # fib_index = min(range(len(levels)), key=lambda i: abs(Close[current_index] - levels[i]))
            if closest_level > Close[current_index]:
                
                fib_index = levels.index(Close[current_index] + abs(closest_level - Close[current_index]))

            
            if Trade_Direction == 1:
                # if fib_index == 6:
                #     take_profit_val = levels[fib_index] / 100
                # else:
                take_profit_val = levels[fib_index + 1] / 100
                stop_loss_val = (levels[0] - (levels[0]*13/100)) / 100
                print("take_profit_val: " + str(take_profit_val))
                print("stop_loss_val: " + str(stop_loss_val))
                print("Level: " + str(len(levels)))
                print("fib_index: " + str(fib_index))
                print(levels)
                ...
                # else:
                take_profit_val = levels[fib_index - 1] / 100
                stop_loss_val = (levels[6] + (levels[6]*13/100)) / 100
                print("take_profit_val: " + str(take_profit_val))
                print("stop_loss_val: " + str(stop_loss_val))
                print("Level: " + str(len(levels)))
                print("fib_index: " + str(fib_index))
            ...
            # if fib_index == 0 and  Trade_Direction == 1 :
            #     # TradeDirection = 1  # Long position
            #     take_profit_val = levels[fib_index + 1]
            #     stop_loss_val = levels[fib_index]
            # elif fib_index == len(ratios) - 1 and  Trade_Direction == 0:
            #     # TradeDirection = 0  # Short position
            #     take_profit_val = levels[fib_index]
            #     stop_loss_val = levels[fib_index - 1]
            # else:
            #     if Close[current_index] > levels[fib_index + 1] and Trade_Direction == 1:
            #         # TradeDirection = 1  # Long position
            #         take_profit_val = levels[fib_index + 1]
            #         stop_loss_val = levels[fib_index]
            #     else:
            ...
            # take_profit_val = levels[take_profit_index]
    ...
    if TP_SL_choice == '%':
        ...
        # # Tính toán giá trị ATR
        # atr = calculate_average_true_range(Close, High, Low)

        # # Tính toán giá trị take profit và stop loss
        # take_profit_val = [TP * atr for _ in range(len(Close))][current_index]
        # stop_loss_val = [SL * atr for _ in range(len(Close))][current_index]


    elif TP_SL_choice == 'x (ATR)':
        # print("take_profit_val: " + str(len(take_profit_val)))
        # print("stop_loss_val: " + str(len(stop_loss_val)))
        take_profit_val = take_profit_val_arr
        stop_loss_val = stop_loss_val_arr
        print("take_profit_val: " + str(take_profit_val))
        print("stop_loss_val: " + str(stop_loss_val))
    # 
        ...
        # Tìm điểm swing cao nhất và thấp nhất
        highest_swing = find_highest_swing(Close, peaks)
        # highest_swing = find_highest_swing(Close, peaks)
        lowest_swing = find_lowest_swing(Close, troughs)

        
        for i in range(current_index - int(TP_SL_choice[-1]), -1, -1):
            if Trade_Direction == 0:
                if peaks[i] > highest_swing and peaks[i] != 0:
                    highest_swing = peaks[i]
                    

            elif Trade_Direction == 1:
                if troughs[i] < lowest_swing and troughs[i] != 0:
                    lowest_swing = troughs[i]


        if Trade_Direction == 0:
            tp_margin = Close[current_index] - lowest_swing
            stop_loss_val = max(0.0, SL * (highest_swing - Close[current_index]))
            take_profit_val = max(0.0, TP * stop_loss_val)
        elif Trade_Direction == 1:
            tp_margin = highest_swing - Close[current_index]
            stop_loss_val = max(0.0, SL * (Close[current_index] - lowest_swing))
            take_profit_val = max(0.0, TP * stop_loss_val)

    elif TP_SL_choice =='x Swing':
        # Implement your trailing stop loss and take profit logic here
        trailing_stop_loss = stop_loss_val_arr[current_index] # Calculate the initial trailing stop loss
        trailing_take_profit = take_profit_val_arr[current_index] # Calculate the initial trailing take profit

        for i in range(current_index, len(Close)):
            # Update the trailing stop loss and take profit levels if a new swing high/low is reached
            if High[i] > peaks[i]:
                trailing_stop_loss = SL * (High[i] - Close[i])
                trailing_take_profit = TP * (High[i] - Close[i])
            elif Low[i] < troughs[i]:
                trailing_stop_loss = SL * (Close[i] - Low[i])
                trailing_take_profit = TP * (Close[i] - Low[i])
            else:
                break

        if Trade_Direction == 0 and Close[i] - Low[i] >= trailing_stop_loss:
            stop_loss_val = Close[i] - Low[i]  # Adjust the stop loss level to the current candle's low
            take_profit_val = trailing_take_profit  # Keep the take profit level the same
        elif Trade_Direction == 1 and High[i] - Close[i] >= trailing_stop_loss:
            stop_loss_val = High[i] - Close[i]  # Adjust the stop loss level to the current candle's high
            take_profit_val = trailing_take_profit  # Keep the take profit level the same
